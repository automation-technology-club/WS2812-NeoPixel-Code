...
void Performance()
{
	...
	FROM(0, 0, 00.002){fill_solid(leds, NUM_LEDS, CRGB::Grey);}
	FROM(0, 0, 2.000){fadeToBlack();	}
	FROM(0, 0, 10.000){confetti();}
	FROM(0, 0, 20.000){juggle();}
	FROM(0, 0, 40.000){rainbowWithGlitter();}
	FROM(0, 1, 00.000){fill_solid(leds, NUM_LEDS, CHSV(gHue, 127, 127));}
	AT(0, 1, 5.000){fill_solid(leds, NUM_LEDS, CRGB::White);}
	AT(0, 1, 10.000){fill_solid(leds, NUM_LEDS, CRGB::Red);}
	AT(0, 1, 15.000){fill_solid(leds, NUM_LEDS, CRGB::Green);}
	AT(0, 1, 20.000){fill_solid(leds, NUM_LEDS, CRGB::Blue);}
	FROM(0, 1, 30.000){fadeToBlack();}
	FROM(0, 1, 35.000){applause();}
	AT(0, 1, 40.000){FastLED.setBrightness(BRIGHTNESS / 2);}
	AT(0, 1, 42.000){FastLED.setBrightness(BRIGHTNESS / 4);}
	AT(0, 1, 44.000){FastLED.setBrightness(BRIGHTNESS / 8);}
	AT(0, 1, 46.000){FastLED.setBrightness(BRIGHTNESS / 16);}
	FROM(0, 1, 48.000){fadeToBlack();}
}
...
void loop()
{
	...
	if (gTimeCode > 120000) RestartPerformance();
